  ...
  async generateCertificateCodeToMethod(methodID: number) {
    try {
      ...
      const lastMethod = await this.NI_MCIT_M_01Repository.createQueryBuilder(
        ...
        .where('EXTRACT(YEAR FROM NI_MCIT_M_01.created_at) = :currentYear', {
          currentYear,
        })
        .orderBy('NI_MCIT_M_01.last_record_index', 'DESC')
        .getOne()
      ...
      await this.dataSource.transaction(async (manager) => {
        ...
        await this.methodService.updateLastRecordIndex('NI_MCIT_M_01')
  ...
  async getCertificateResult(methodID: number, activityID: number) {
    try {
      ...
      return handleOK({
        ...
        environmental_conditions: {
          temperature: `Temperatura: ${formatNumberCertification(Number(sheet.cell('E62').value()))} °C ± ${formatNumberCertification(Number(sheet.cell('G62').value()))} °C`,
          humidity: `Humedad Relativa: ${formatNumberCertification(Number(sheet.cell('E63').value()))} % ± ${formatNumberCertification(Number(sheet.cell('G63').value()))} % RH`,
          presion: `Presión: ${formatNumberCertification(Number(sheet.cell('R62').value()))} hPa ± ${formatNumberCertification(Number(sheet.cell('U62').value()))} hPa`,
        },
...
Este certificado de calibración no debe ser reproducido sin la aprobación del laboratorio, excepto cuando se reproduce en su totalidad.`,
  ...
  async getAnnotationSheetData(methodID: number): Promise<NI_MCIT_M_01> {
    try {
      const method = await this.NI_MCIT_M_01Repository.findOne({
        where: { id: methodID },
        relations: [
